...
TABLES: dict[str, list[bigquery.SchemaField]] = {
    ...
    "sharepoint_files": [
        bigquery.SchemaField("file_id",       "STRING",    mode="REQUIRED",    description="MD5 hash of file contents — stable across renames/moves"),
        bigquery.SchemaField("file_name",     "STRING",    mode="REQUIRED",    description="Filename"),
        bigquery.SchemaField("file_path",     "STRING",    mode="REQUIRED",    description="Full local path at time of scan"),
        bigquery.SchemaField("source_folder", "STRING",    mode="NULLABLE",    description="Named source folder (e.g. 'Secondary Resource Library')"),
        bigquery.SchemaField("doc_date",      "DATE",      mode="NULLABLE",    description="Document date parsed from filename or first-page content"),
        bigquery.SchemaField("date_source",   "STRING",    mode="NULLABLE",    description="'filename' | 'content' | 'unknown'"),
        bigquery.SchemaField("file_size",     "INT64",     mode="NULLABLE",    description="File size in bytes"),
        bigquery.SchemaField("file_ext",      "STRING",    mode="NULLABLE",    description="pdf / pptx / xlsx etc."),
        bigquery.SchemaField("ingested",      "BOOL",      mode="NULLABLE",    description="True once fed through the pipeline"),
        bigquery.SchemaField("report_id",     "STRING",    mode="NULLABLE",    description="FK to reports.report_id after ingestion"),
        bigquery.SchemaField("ingested_at",   "TIMESTAMP", mode="NULLABLE",    description="When ingestion completed"),
...
TABLE_CONFIG: dict[str, dict] = {
    ...
    "sharepoint_files": {"partition_field": "first_seen", "partition_type": "MONTH", "clustering_fields": ["ingested", "doc_date"]},
